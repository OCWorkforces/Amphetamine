...
| Package   | Bun                                       |
| Test      | Vitest 4 (workspace, 107 tests)           |
...
│   ├── ipc.ts            # IPC handlers (12 channels, typed)
...
└── shared/             # Types shared across processes
    └── types.ts          # IPC_CHANNELS (12), AppSettings, IpcChannelMap
```
...
| `registerIpcHandlers`        | fn    | src/main/ipc.ts:88             | IPC registration (12 channels)                                                 |
| `typedHandle`                | fn    | src/main/ipc.ts:78             | Type-safe IPC wrapper                                                          |
| `validateSender`             | fn    | src/main/ipc.ts:38             | Origin validation                                                              |
...
| `cancelSession`              | fn    | src/main/session-timer.ts:70   | Cancel active session                                                          |
| `getStatus`                  | fn    | src/main/session-timer.ts:87   | Get current session state                                                      |
| `cleanup`                    | fn    | src/main/session-timer.ts:113  | Clear timer without syncing sleep                                              |
| `syncPreventSleep`           | fn    | src/main/power-saver.ts:53     | Sync sleep blocker state                                                       |
| `initBatteryMonitoring`      | fn    | src/main/power-saver.ts:81     | Battery drain auto-disable                                                     |
...
| `setBatteryAutoStopCallback` | fn    | src/main/power-saver.ts:73     | Wire battery auto-stop to session cancel                                       |
...
| `initAutoUpdater`            | fn    | src/main/auto-updater.ts:30    | Register electron-updater event handlers                                       |
| `stopAutoUpdater`            | fn    | src/main/auto-updater.ts:125   | Stop auto-updater + clear interval                                             |
| `registerAutoUpdaterIpc`     | fn    | src/main/auto-updater.ts:138   | IPC handler for manual update check                                            |
...
- **Dev env var**: `DEV_SERVER_URL` (set by `scripts/dev.ts` for dev server URL)
...
bun run test           # Run Vitest tests (107 tests, 2 workspaces)
...
- `flip-fuses.cjs`: Flips Electron fuses post-build (EnableCookieEncryption, EnableFuses)
...
| -------- | ----- | ----- | ------------------------------------------------------------------------------------ |
| main     | node  | 107   | Session timer, IPC, power-saver, settings, tray, shortcut, auto-launch, auto-updater |
...
- **CI**: GitHub Actions configured (ci.yml + cd.yml)
...
- `.github/workflows/`: CI/CD workflows configured (ci.yml + cd.yml)
